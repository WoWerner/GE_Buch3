select
  ...
  konten.Name,
  konten.PlanSumme,
  ifnull((Select sum(journal.Betrag) from Journal where (journal.konto_nach = konten.KontoNr) and (journal.BuchungsJahr = :BJAHR)    ), 0) as Summe_dieses_Jahr,
...
where (konten.kontotype = :TYP) and 
      (konten.Statistik <> 99) :AddWhereAnd and 
